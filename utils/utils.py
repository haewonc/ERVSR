...
def print_psnr(state, epoch=None, max_epoch=None, mean_loss=None):
    CURSOR_UP_ONE = '\x1b[1A' 
    ERASE_LINE = '\x1b[2K'

    if state == "TEST":
        sys.stdout.write('[{}] {} {}\n'.format(
                toWhite('{} TOTAL'.format(state)),
                toWhite('PSNR'),
                mean_loss
                )
            )
...
def print_logs(state, epoch=None, max_epoch=None, vid_iter=None, total_vids=None, time=None, loss=None):
    CURSOR_UP_ONE = '\x1b[1A' 
    ERASE_LINE = '\x1b[2K'
...
def get_patch(LR_UW, LR_W, LR_T, HR_UW, HR_W=None, is_crop=False, patch_size=64, scale=4, flag_HD_in=False, is_train=True):
